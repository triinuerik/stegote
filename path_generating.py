...
def generate_zigzag_dct_path(dct_coefficients_image):
    ...
    the quantized DCT coefficients. The path is only constructed from non-zero coefficients. The path needs to be sent
    to the receiver in order to decode the message (in an encrypted manner using encrypt_path()).
...
def generate_zigzag_coordinate_dictionary(cover_image):
    index = 0
    dictionary = dict()
    ...
    if image_is_greyscale(cover_image):
        for x in range(len(cover_image[0, :, 0])):
            for y in range(len(cover_image[:, 0, 0])):
                dictionary[index] = [y, x]
                index += 1
    else:
        for z in range(len(cover_image[0, 0, :])):
            for x in range(len(cover_image[0, :, 0])):
                for y in range(len(cover_image[:, 0, 0])):
                    dictionary[index] = [y, x, z]
                    index += 1
